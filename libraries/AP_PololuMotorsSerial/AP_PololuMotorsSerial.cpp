...
void AP_PololuMotorsSerial::accelerate_motors(unsigned int leftWheelDirection,
		unsigned int leftWheelVelocity, unsigned int rightWheelDirection,
		unsigned int rightWheelVelocity) {

	unsigned int command_byte;
	command_byte = ACCL_MOTOR_BOTH | (rightWheelDirection * 4)
			| leftWheelDirection;
	SEND_CMD(command_byte);
	SEND_CMD(leftWheelVelocity);
	SEND_CMD(rightWheelVelocity);
...
void AP_PololuMotorsSerial::move_forward(unsigned int wheel,
...
void AP_PololuMotorsSerial::move_reverse(unsigned int wheel,
		unsigned int velocity) {
...
void AP_PololuMotorsSerial::accelerate_forward(unsigned int wheel,
		unsigned int velocity) {
...
void AP_PololuMotorsSerial::set_target_velocity(unsigned int leftDirection,
		unsigned int left, unsigned rightDirection, unsigned int right) {
...
void AP_PololuMotorsSerial::god_speed() {
	...
	dVLeft = kp
			* (targetLeftVelocity * targetLeftDirection * 7614.0 / 127.0
					- curLeft);
	dVRight = kp
			* (targetRightVelocity * targetRightDirection * 7614.0 / 127.0
					- curRight);
