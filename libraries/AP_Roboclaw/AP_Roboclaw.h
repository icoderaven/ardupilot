...
#ifndef __AP_Roboclaw_H__
#define __AP_Roboclaw_H__

#define ROBOCLAW_SERIAL_RATE 38400 //default
#define MAX_SPEED	10000
#define MAX_SPEED_HALF  5000

//TO Roboclaw motor controller using UART1 reserved for GPS on ardupilot board
#define SEND_CMD(x)         hal.uartB->write(x)
#define ADDRESS 0x80
...
public:
	...
	//supporting minimum functionality 
	bool ResetEncoders(uint8_t address);
	bool SetM1VelocityPID(uint8_t address, float Kd_, float Kp_, float Ki_, uint32_t qpps_);
	bool SetM2VelocityPID(uint8_t address, float Kd_, float Kp_, float Ki_, uint32_t qpps_);
...
private:
	//temporary support for write only
