...
//@Rover
static void NOINLINE send_odometry(mavlink_channel_t chan)
{
	//Using the global vision position estimate MAVlink message for this
...
//@Rover
static void NOINLINE send_scaled_imu(mavlink_channel_t chan)
{
	//I'm going to send the AHRS filtered RPY values here in place of the gyro and compass values
	Vector3f accel = ins.get_accel();
	Vector3f omega = ahrs.get_gyro();
...
static bool mavlink_try_send_message(mavlink_channel_t chan, enum ap_message id,
	...
	int16_t payload_space = comm_get_txspace(chan)
	...
	case MSG_SCALED_IMU:
		...
		send_scaled_imu(chan);
		break;

	case MSG_GLOBAL_VISION_POSITION_ESTIMATE:
		CHECK_PAYLOAD_SIZE(GLOBAL_VISION_POSITION_ESTIMATE);
		send_odometry(chan);
		break;
	//@Rover}
...
void mavlink_send_text(mavlink_channel_t chan, gcs_severity severity,
		const char *str) {
	if (telemetry_delayed(chan)) {
		return;
	}
	...
	if (severity == SEVERITY_LOW) {
		...
		mavlink_statustext_t *s = (
				chan == MAVLINK_COMM_0 ?
						&gcs0.pending_status : &gcs3.pending_status);
...
GCS_MAVLINK::GCS_MAVLINK() :
		packet_drops(0), waypoint_send_timeout(1000), // 1 second
...
void GCS_MAVLINK::update(void) {
	...
	if (waypoint_receiving && waypoint_request_i <= waypoint_request_last
			&& tnow
					> waypoint_timelast_request + 500
							+ (stream_slowdown * 20)) {
...
#else
		//Acceleration+Filtered orientation estimate
		send_message(MSG_SCALED_IMU);
		//Odometry data
		send_message(MSG_GLOBAL_VISION_POSITION_ESTIMATE);
		//Raw 9DoF IMU data
		send_message(MSG_RAW_IMU1);
#endif
	}
...
void GCS_MAVLINK::handleMessage(mavlink_message_t* msg) {
	...
	case MAVLINK_MSG_ID_COMMAND_LONG: {
		...
		case MAV_CMD_PREFLIGHT_CALIBRATION:
			if (packet.param1 == 1 || packet.param2 == 1
					|| packet.param3 == 1) {
	...
	case MAVLINK_MSG_ID_MISSION_REQUEST_LIST: {
		...
		mavlink_msg_mission_count_send(chan, msg->sysid, msg->compid,
	...
	case MAVLINK_MSG_ID_MISSION_REQUEST: {
		...
		mavlink_msg_mission_item_send(chan, msg->sysid, msg->compid, packet.seq,
				frame, tell_command.id, current, autocontinue, param1, param2,
				param3, param4, x, y, z);
	...
	case MAVLINK_MSG_ID_PARAM_REQUEST_READ: {
		...
		if (packet.param_index != -1) {
			...
			if (vp == NULL) {
				gcs_send_text_fmt(PSTR("Unknown parameter index %d"),
						packet.param_index);
				break;
			}
			...
			if (vp == NULL) {
				gcs_send_text_fmt(PSTR("Unknown parameter %.16s"),
						packet.param_id);
				break;
			}
		}
		...
		mavlink_msg_param_value_send(chan, param_name, value,
				mav_var_type(p_type), _count_parameters(), packet.param_index);
		break;
	}
	...
	case MAVLINK_MSG_ID_MISSION_CLEAR_ALL: {
		...
		mavlink_msg_mission_ack_send(chan, msg->sysid, msg->compid,
	...
	case MAVLINK_MSG_ID_MISSION_ITEM: {
		...
		if (packet.current == 2) { //current = 2 is a flag to tell us this is a "guided mode" waypoint and not for the mission
			...
			mavlink_msg_mission_ack_send(chan, msg->sysid, msg->compid, 0);
	...
	case MAVLINK_MSG_ID_PARAM_SET: {
		...
		if ((NULL != vp) &&                             // exists
			...
			mavlink_msg_param_value_send(chan, key, vp->cast_to_float(var_type),
					mav_var_type(var_type), _count_parameters(), -1); // XXX we don't actually know what its index is...
	...
	default:
		...
		if ((chan == MAVLINK_COMM_1 && gcs0.initialised)
			...
			mavlink_channel_t out_chan = (mavlink_channel_t) (((uint8_t) chan)
					^ 1);
			// only forward if it would fit in our transmit buffer
			if (comm_get_txspace(out_chan)
...
uint16_t GCS_MAVLINK::_count_parameters() {
...
void GCS_MAVLINK::queued_param_send() {
	...
	bytes_allowed = g.serial3_baud * (tnow - _queued_parameter_send_time_ms)
			* 26;
	...
	while (_queued_parameter != NULL && count--) {
		...
		vp->copy_name_token(_queued_parameter_token, param_name,
				sizeof(param_name), true);

		mavlink_msg_param_value_send(chan, param_name, value,
				mav_var_type(_queued_parameter_type), _queued_parameter_count,
...
void GCS_MAVLINK::queued_waypoint_send() {
	if (waypoint_receiving && waypoint_request_i <= waypoint_request_last) {
		mavlink_msg_mission_request_send(chan, waypoint_dest_sysid,
				waypoint_dest_compid, waypoint_request_i);
	}
}
